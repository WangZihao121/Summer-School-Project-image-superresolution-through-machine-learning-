...
class FSRCNN(nn.Module):
    def __init__(self, scale_factor, num_channels=1, d=56, s=12, m=4):      #__init__()只在创建模型时执行一次
        ...
        # 特征提取层：使用 5x5 卷积提取图像初始特征，通道数从 num_channels 转为 d
        self.first_part = nn.Sequential(
            nn.Conv2d(num_channels, d, kernel_size=5, padding=5//2),
            nn.PReLU(d)
        )        #这行代码创建了一个顺序容器 nn.Sequential，它是nn.Module的一个子类，你可以把它理解成一个“流水线”，数据会依次流过每一层。
                 #self.first_part是nn.Sequential这个类的实例化。
                 #此处不能理解为nn.Sequential这个类继承了nn.Conv2d和nn.PReLU这两个类
        # 特征收缩与非线性映射层：先压缩通道数，再通过多个 3x3 卷积进行深度映射
